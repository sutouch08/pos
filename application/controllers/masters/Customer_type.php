...
class Customer_type extends PS_Controller
{
  ...
  public function index()
  {
    $filter = array(
      ...
      'order_by' => get_filter('order_by', 'customer_type_order_by', 'code'),
      'sort_by' => get_filter('sort_by', 'customer_type_sort_by', 'ASC')
    );
  ...
  public function is_exists_code()
  {
    ...
    if (! empty($ds))
    {
      if ($this->customer_type_model->is_exists_code($ds->code, $ds->id))
      {
        echo 'exists';
      }
      else
  ...
  public function add()
  {
    ...
    if ($this->pm->can_add)
    {
      ...
      {
        if ($this->customer_type_model->is_exists_code($ds->code))
        {
          $sc = FALSE;
        ...
        if ($sc === TRUE)
        {
          $res = $this->customer_type_model->get_by_code($ds->code);
  ...
  public function get_data()
  {
    ...
    {
      $res = $this->customer_type_model->get_by_id($ds->id);
    ...
    if ($this->pm->can_edit)
    {
      if (! empty($ds) && ! empty($ds->id) && ! empty($ds->code) && ! empty($ds->name))
      {
        ...
        if ($sc === TRUE)
        {
          ...
          if (! $this->customer_type_model->update($ds->id, $arr))
          {
            $sc = FALSE;
            set_error('update');
          }
  ...
  public function delete()
  {
    ...
    if ($this->pm->can_delete)
    {
      if (! empty($ds) && ! empty($ds->id))
      {
        $item = $this->customer_type_model->get_by_id($ds->id);

        if (! empty($item))
        {
          if ($this->customer_type_model->count_members($ds->id) > 0)
          {
            $sc = FALSE;
          ...
          if ($sc === TRUE)
          {
            if (! $this->customer_type_model->delete($ds->id))
            {
              $sc = FALSE;
              set_error('delete');
            }
  ...
  {
    $filter = ['customer_type_code', 'customer_type_order_by', 'customer_type_sort_by'];
