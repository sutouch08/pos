...
class Product_size_group extends PS_Controller
{
  public $menu_code = 'DBPSIG';
  public $menu_group_code = 'DB';
  public $menu_sub_group_code = 'PRODUCT';
  public $title = 'เพิ่ม/แก้ไข กลุ่มไซส์';
  public $segment = 4;

  public function __construct()
  {
    parent::__construct();
    $this->home = base_url() . 'masters/product_size_group';
    $this->load->model('masters/product_size_group_model');
  ...
  public function index()
  {
    ...
    {
      $perpage = get_rows();
      $rows = $this->product_size_group_model->count_rows($filter);
      $filter['data'] = $this->product_size_group_model->get_list($filter, $perpage, $this->uri->segment($this->segment));

      if( ! empty($filter['data']))
      {
        foreach($filter['data'] as $rs)
        {
          $rs->member = $this->product_size_group_model->count_members($rs->id);
        }
      }
      
      $init  = pagination_config($this->home . '/index/', $rows, $perpage, $this->segment);
      $this->pagination->initialize($init);
      $this->load->view('masters/product_size_group/product_size_group_list', $filter);
    }
  }
  ...
  public function add()
  {
    ...
    if (! empty($ds) && ! empty($ds->name))
    {
      if ($this->pm->can_add)
      {        
        if ($this->product_size_group_model->is_exists_name($ds->name))
        {
          $sc = FALSE;
        ...
        if ($sc === TRUE)
        {
          ...
          if ($sc === TRUE)
          {
            $res = $this->product_size_group_model->get($id);

            if (! empty($res))
            {
              $res->member = $this->product_size_group_model->count_members($id);
  ...
  public function update()
  {
    ...
    if (! empty($ds) && ! empty($ds->id) && ! empty($ds->name))
    {
      if ($this->pm->can_edit)
      {       
        if ($this->product_size_group_model->is_exists_name($ds->name, $ds->id))
        {
          $sc = FALSE;
          set_error('exists', $ds->name);
        }
  ...
  public function delete()
  {
    ...
    if (! empty($ds) && ! empty($ds->id))
    {
      ...
      {
        $member = $this->product_size_group_model->count_members($ds->id);
        ...
        if($sc === TRUE)
        {
          if (! $this->product_size_group_model->delete($ds->id))
          {
            $sc = FALSE;
            set_error('delete');
          }
  ...
  public function get_data()
  {
    ...
    if (! empty($ds) && ! empty($ds->id))
    {
      $data = $this->product_size_group_model->get($ds->id);      
      if(empty($data))
      {
        $sc = FALSE;
        set_error('not_found');
