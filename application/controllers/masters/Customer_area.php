...
class Customer_area extends PS_Controller
{
  ...
  public function is_exists_code()
  {
    ...
    if (! empty($ds))
    {
      if ($this->customer_area_model->is_exists_code($ds->code, $ds->id))
      {
        echo 'exists';
      }
      else
  ...
  public function add()
  {
    ...
    if ($this->pm->can_add)
    {
      ...
      {
        if ($this->customer_area_model->is_exists_code($ds->code))
        {
          $sc = FALSE;
        ...
        if ($sc === TRUE)
        {
          $res = $this->customer_area_model->get_by_code($ds->code);
  ...
  public function get_data()
  {
    ...
    {
      $res = $this->customer_area_model->get_by_id($ds->id);
  ...
  public function delete()
  {
    ...
    if ($this->pm->can_delete)
    {
      if (! empty($ds) && ! empty($ds->id))
      {
        $item = $this->customer_area_model->get_by_id($ds->id);

        if (! empty($item))
        {
          if ($this->customer_area_model->count_members($ds->id) > 0)
          {
            $sc = FALSE;
          ...
          if ($sc === TRUE)
          {
            if (! $this->customer_area_model->delete($ds->id))
            {
              $sc = FALSE;
              set_error('delete');
            }
