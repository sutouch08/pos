...
class Stock_balance_zone extends PS_Controller
{
  ...
  public function do_export()
  {
    ...
    $this->excel->getActiveSheet()->setCellValue('G5', 'รุ่น');
    $this->excel->getActiveSheet()->setCellValue('H5', 'สี');
    $this->excel->getActiveSheet()->setCellValue('I5', 'ไซส์');
    $this->excel->getActiveSheet()->setCellValue('J5', 'ทุน');
    $this->excel->getActiveSheet()->setCellValue('K5', 'จำนวน');
    $this->excel->getActiveSheet()->setCellValue('L5', 'มูลค่า');
    ...
    if(!empty($result))
    {
      ...
      foreach($result as $rs)
      {
        ...
        $this->excel->getActiveSheet()->setCellValue("G{$row}", $item->style_code);
        $this->excel->getActiveSheet()->setCellValue("H{$row}", $item->color_code);
        $this->excel->getActiveSheet()->setCellValue("I{$row}", $item->size_code);
        $this->excel->getActiveSheet()->setCellValue("J{$row}", $rs->price);
        $this->excel->getActiveSheet()->setCellValue("K{$row}", $rs->qty);
        $this->excel->getActiveSheet()->setCellValue("L{$row}", "=G{$row}*H{$row}");
        $no++;
        $row++;
      }
      ...
      $this->excel->getActiveSheet()->setCellValue('K'.$row, "=SUM(K6:K{$ro})");
      $this->excel->getActiveSheet()->setCellValue('L'.$row, "=SUM(L6:L{$ro})");
