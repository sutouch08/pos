...
class Consign_order extends PS_Controller
{
  ...
  public function index()
  {
    $filter = array(
      ...
      'channels' => get_filter('channels', 'consign_chanels', 'all'),
      'sale_id' => get_filter('sale_id', 'consign_sale_id', 'all'),
      ...
      'so_code' => get_filter('so_code', 'consign_so_code', '')
    );
  ...
  public function cancle($code)
  {
    ...
    if($this->pm->can_delete)
    {
	      ...
	      else
	      {
	        ...
	        if(! $this->consign_order_model->cancle_details($code))
	        {
	          $sc = FALSE;
          ...
          if($sc === TRUE && $doc->ref_type == 4)
          {
            ...
            //--- roll back items status to open

            $bills = $this->order_pos_model->get_bills_by_ref_code($code);
            ...
            if( ! empty($bills))
            {
              foreach($bills as $rs)
              {
                ...
                if( ! $this->order_pos_model->update_details_by_id($rs->id, array('status' => 'O')))
                {
                  $sc = FALSE;
                  $this->error = "Failed to rollback bill item status";
                }

                if($sc === TRUE)
                {
                  if( ! $this->order_pos_model->update_by_id($rs->id, array('ref_code' => NULL, 'status' => 'O')))
                  {
                    $sc = FALSE;
                    $this->error = "Failed to rollback bill status";
                  }
                }
              }
            }
  ...
  public function save_consign($code)
  {
    ...
    $this->error = "<span>ยอดในโซนไม่พอตัด</span><br/>";
        ...
        foreach($details as $rs)
        {
          ...
          if(!empty($item))
          {
            ...
            else
            {
              ...
              if(empty($errList[$pid]))
              {
                $errList[$pid] = $all_qty;
                $this->error .= "<span>{$item->code} ในโซน: {$stock} ยอดตัด : {$all_qty} </span><br/>";
              }
            }
          }
          else
                  ...
                  if($item->active)
                  {
                    ...
                    else
                    {
                      ...
                      if($new_qty <= $stock OR $auz === TRUE)
                      {
                        ...
                        if($sc === TRUE)
                        {
                          $this->consign_order_model->update($doc->code, array('ref_type' => 3));
                        }
                      }
                      else
                      {
  ...
  public function remove_import_details($code)
  {
    ...
    if($this->input->post('check_code'))
    {
      ...
      if( ! $saved)
      {
        ...
        //--- delete details
        if( ! $this->consign_order_model->drop_import_details($code, $check_code))
        {
          $sc = FALSE;
          $this->error = "Failed to drop imported items";
        }

        //--- update ref_code
        if($sc === TRUE)
        {
          if( ! $this->consign_order_model->update($code, array('ref_code' => NULL, 'ref_type' => 1)))
          {
            $sc = FALSE;
            $this->error = "Failed to remove reference";
          }
        }
        ...
        if($sc === TRUE)
        {
          if( ! $this->consign_check_model->update_ref_code($check_code, NULL, 0))
          {
            $sc = FALSE;
            $this->error = "Failed to unlink consolidate document";
          }
        }
  ...
  public function export_consign_update($code)
  {
    $rs = $this->export_consign_sold($code, 'U');
  ...
  public function export_consign_sold($code, $option = 'A')
  {
    $sc = TRUE;
    $this->load->library('export');

    if(! $this->export->export_consign_order($code, $option))
    {
      $sc = FALSE;
