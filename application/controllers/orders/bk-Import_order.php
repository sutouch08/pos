        ...
        else
        {
          ...
          if($sc === TRUE && ! empty($rs['A']))
          {
            ...
            if( ! isset($ds[$ref_code]))
            {
              ...
              $warehouse_code = empty(trim($rs['L'])) ? $default_warehouse : trim($rs['L']);
              ...
              if($sc === TRUE)
              {
                ...
                //--- ค่าจัดส่ง
                $shipping_fee = empty($rs['K']) ? 0.00 : trim($rs['K']);
                $shipping_fee = is_numeric($shipping_fee) ? $shipping_fee : 0.00;

                //-- remark
                $remark = get_null(trim($rs['S']));

                $qty = empty(trim($rs['H'])) ? 1 : str_replace(',', '', $rs['H']);
                $qty = is_numeric($qty) ? $qty : 1;

                //--- ราคา
                $price = empty($rs['I']) ? 0.00 : str_replace(",", "", $rs['I']);
                ...
                $ds[$ref_code] = (object) array(
                  ...
                  'remark' => $remark,
                  'vat_type' => $vat_type,
                  'address' => get_null(trim($rs['M'])),
                  'sub_district' => get_null(trim($rs['N'])),
                  'district' => get_null(trim($rs['O'])),
                  'province' => get_null(trim($rs['P'])),
                  'postcode' => get_null(trim($rs['Q'])),
                  'phone' => get_null(trim($rs['R'])),
                  'force_update' => (trim($rs['T']) == 'Y' OR trim($rs['T']) == 'y') ? TRUE : FALSE,
                  'items' => array()
                );
