...
class Iprinter
{
	...
	public function doc_header($pageTitle = 'print pages')
	{
		...
		$header .= "	<link href='".base_url()."assets/css/font-sarabun.css' rel='stylesheet' />";
	...
	public function thead(array $dataset)
	{
		...
		$thead 	.= "<thead>";
		$thead	.= "<tr style='height:".$this->row_height."mm; line-height:".$this->row_height."mm; font-size:".$this->font_size."px;'>";
	...
	{
		$content = "<div style='width:".$this->content_width."mm; margin:auto; margin-bottom:1mm; border:solid 2px #333; border-radius: 10px;' >";
		$content .= $data;
		$content .="</div>";
		return $content;
	}
	...
	{
		$border = $this->content_border == 0 ? 'border:none;' : 'border:solid 1px #333;';
		$content_height = $this->content_height == 0 ? "" : "height:{$this->content_height}mm;";
		return  "<div style='width:{$this->content_width}mm; {$content_height} margin:auto; margin-bottom:1mm; border-radius: 10px; {$border}'>";
	}
