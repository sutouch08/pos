...
class Order_grid
{
  ...
  public function getProductGrid($style_code, $view = FALSE, $whsCode = NULL, $zoneCode = NULL)
	{
		$sc = '';
    ...
    if(!empty($style))
    {
      ...
      if( count($attrs) == 1  )
      {
        $sc .= $this->orderGridOneAttribute($style, $attrs[0], $view, $whsCode, $zoneCode);
      }
      else if( count( $attrs ) == 2 )
      {
        $sc .= $this->orderGridTwoAttribute($style, $view, $whsCode, $zoneCode);
      }
