...
function getEditTabsTree($id, $script = TRUE, $active = TRUE)
{
	$cs =& get_instance();
	...
	foreach( $qs->result() as $rs )
	{
		if( $rs->id != $id )
		{
			...
			if( hasEditChild($rs->id, $id) === TRUE )
			{
				...
				$sc .= '<label class="padding-10">';
				$sc .= '<input type="radio" class="ace" name="tabs" value="'.$rs->id.'" '. is_checked($id_parent, $rs->id) .' '.$disabled.'/>';
				$sc .= '<span class="lbl">'.$rs->name.'</label>';
				$sc .= '<ul id="edit-catchild-'.$rs->id.'" class="'.$ex.'">';
				$sc .= getEditChild($rs->id, $parent, $id_parent, $id, $active) ;
				$sc .= '</ul>';
			}
			else
			{
...
function getEditChild($id_parent, $parent, $id, $id_tab, $active = TRUE)
{
	$disabled = $active == FALSE ? 'disabled' : '';
	$sc = '';
	$ci =& get_instance();
		...
		foreach( $qs->result() as $rs)
		{
			if( $rs->id != $id_tab )
			{
				...
				if( hasEditChild($rs->id, $id_tab) === TRUE )
				{
					...
					$sc .= '<i class="fa fa-'.$ep.'-square-o" id="edit-catbox-'.$rs->id.'" onClick="toggleEditTree('.$rs->id.')"></i>';
					$sc .= '<label class="padding-10">';
					$sc .= '<input type="radio" class=" ace margin-right-10" name="tabs" value="'.$rs->id.'" '. is_checked($id, $rs->id) .' '.$disabled.'/>';
					$sc .= '<span class="lbl">'.$rs->name. '</span>';
					$sc .= '</label>';
					$sc .= '<ul id="edit-catchild-'.$rs->id.'" class="'.$ex.'">';
					$sc .= getEditChild($rs->id, $parent, $id, $id_tab, $active) ;
					$sc .= '</ul>';
				}
				else //-- if hasChild
				{
...
function productTabsTree($style_code = '', $script = TRUE, $active = TRUE)
{
	$cs =& get_instance();
	$cs->load->model('masters/product_tab_model');
	//----- รายการที่ติ๊ก
	...
	if($qs->num_rows() > 0)
	{
		foreach( $qs->result() as $rs )
		{
			...
			if( hasChild($rs->id) === TRUE )
			{
				...
				$sc .= '<label class="padding-10">
									<input type="checkbox" class="ace margin-right-10" name="tabs[]" value="'.$rs->id.'" '. $is_checked .' '.$disabled.'/>
									<span class="lbl">'.$rs->name.'</span></label>';
				$sc .= '<ul id="catchild-'.$rs->id.'" class="'.$ex.'">';
				$sc .= productTabChild($rs->id, $style_code, $parent, $se, $active) ;
				$sc .= '</ul>';
			}
			else
			{
...
function productTabChild($id_parent, $style_code, $parent, $se, $active = TRUE )
{
	$disabled = $active === FALSE ? 'disabled' : '';
	$sc = '';
	$ci =& get_instance();
	...
	if( $qs->num_rows() > 0 )
	{
		...
		foreach( $qs->result() as $rs)
		{
			...
			if( hasChild($rs->id) === TRUE )
			{
				$sc .= '<i class="fa fa-'.$ep.'-square-o" id="catbox-'.$rs->id.'" onClick="toggleTree('.$rs->id.')"></i>';
				$sc .= '<label class="padding-10">
									<input type="checkbox" class="ace" name="tabs[]" value="'.$rs->id.'" '. $is_checked .' '.$disabled.'/>
