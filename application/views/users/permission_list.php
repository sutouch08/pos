...
<form id="search-form" method="post" action="<?php echo current_url(); ?>">
	<div class="row">
		...
		<div class="col-lg-3 col-md-4 col-sm-4 col-xs-6 padding-5">
			<label>Menu</label>
			<select class="form-control input-sm filter" id="menu-x" name="menu">
				<option value="all">ทั้งหมด</option>
				<?php $groups = $this->menu_model->get_active_menu_groups();		?>
				<?php if (!empty($groups)) : ?>
					<?php foreach ($groups as $group) : ?>
						<?php if ($group->pm == 1) : ?>
							<?php $menu_list = $this->menu_model->get_valid_menus_by_group($group->code); ?>
		...
		<div class="col-lg-2 col-md-2 col-sm-2 col-xs-6 padding-5">
			...
			<select class="form-control input-sm filter" name="permission" id="permission">
				<option value="all">ทั้งหมด</option>
