...
<div class="row">
	<div class="col-lg-8 col-md-8 col-sm-8 col-xs-6 padding-5">
		<p class="red">** เชื่อมโยงสินค้าได้เฉพาะสถานะ "รอดำเนินการ" เท่านั้น</p>
	</div>
	<div class="col-lg-4 col-md-4 col-sm-4 col-xs-6 padding-5">
	<?php if($order->state == 1) : ?>
		<button type="button" class="btn btn-xs btn-danger btn-100 pull-right" onclick="removeChecked()">ลบรายการ</button>
	<?php endif; ?>
	</div>
			...
			<tr class="font-size-10" id="row_<?php echo $rs->id; ?>">
				<td class="middle text-center">
					<?php if(empty($approve_view)) : ?>
						<?php if( $order->is_expired == 0 && $order->is_approved == 0 && ($this->pm->can_edit OR $this->pm->can_add) && $order->state == 1) : ?>
							<label>
