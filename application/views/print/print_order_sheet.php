...
$row_per_page = 13; //--- จำนวนบรรทัด/หน้า
$total_row 	= floor($order->total_rows);
//$all_row = empty($details) ? 0 : count($details);
$row_span = 3;
...
while($total_page > 0 )
{
  ...
  if($this->xprinter->current_page == $this->xprinter->total_page)
  {
    $qty  = "<b>*** จำนวนรวม  ".number($total_qty)."  หน่วย ***</b>";
		$totalBfDisc = number($total_order, 2);
		$discAmount = number(($total_discount + $bill_discount),2);
		$net_amount = ($total_amount + $order->shipping_fee + $order->service_fee) - $bill_discount;
