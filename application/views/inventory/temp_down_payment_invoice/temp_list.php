        ...
        <tr class="font-size-12" id="row-<?php echo $no; ?>">
					<td class="">
						<button type="button"
							...
							onclick="viewDetail(<?php echo $rs->DocEntry; ?>)">
							<i class="fa fa-eye"></i>
						</button>
						<?php if(($rs->F_Sap === 'N' OR $rs->F_Sap == NULL)) : ?>
							<button type="button"
								class="btn btn-minier btn-danger"
								onclick="deleteTemp(<?php echo $rs->DocEntry; ?>, '<?php echo $rs->U_ECOMNO; ?>', <?php echo $no; ?>)">
          ...
          <td class=""><a href="javascript:void(0)" onclick="viewInvoice('<?php echo $rs->U_ECOMNO; ?>')"><?php echo $rs->U_ECOMNO; ?></a></td>
...
<script src="<?php echo base_url(); ?>scripts/inventory/temp/temp_down_payment_invoice.js?v=<?php echo date('YmdH'); ?>"></script>
