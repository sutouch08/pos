...
<form id="search-form" method="post" action="<?php echo current_url(); ?>">
	<div class="row">
		...
		<div class="col-lg-2-harf col-md-3 col-sm-3-harf col-xs-12 padding-5">
			<label>พนักงาน</label>
			<select class="form-control input-sm filter" name="emp_id" id="emp-id">
				<option value="all" <?php echo is_selected('all', $emp_id); ?>>ทั้งหมด</option>
...
<?php if ($this->pm->can_add) : ?>
	<?php $this->load->view('masters/saleman/saleman_add_control'); ?>
<?php endif; ?>
		...
		<table class="table table-striped tableFixHead dataTable border-1">
			<thead>
				<tr>
					...
					<th class="fix-width-150 sorting <?php echo $sort_user; ?>" id="sort-update_user" onclick="sort('update_user', '<?php echo $sort_user; ?>')">แก้ไขโดย</th>
				</tr>
			</thead>
			<tbody id="data-table">
				<?php if (!empty($data)) : ?>
...
<script id="edit-row-template" type="text/x-handlebarsTemplate">
	<tr id="edit-row-{{id}}">
      ...
      <button type="button" class="btn btn-minier btn-default" onclick="cancel({{id}})">
		...
		<td class="middle">
			<select id="employee-{{id}}" class="form-control input-sm">
