...
<?php if ($this->pm->can_add) : ?>
  <?php $this->load->view('masters/product_gender/product_gender_control'); ?>
<?php endif; ?>
    ...
    <table class="table table-striped tableFixHead dataTable border-1">
      <thead>
        <tr>
          ...
          <th class="fix-width-150 middle text-center sorting <?php echo $sort_update; ?>" id="sort-date_upd" onclick="sort('date_upd', '<?php echo $sort_update; ?>')">วันที่ปรับปรุง</th>
          <th class="fix-width-150 middle text-center sorting <?php echo $sort_user; ?>" id="sort-update_user" onclick="sort('update_user', '<?php echo $sort_user; ?>')">ผู้ปรับปรุง</th>
        </tr>
      </thead>
        ...
        <?php if (!empty($data)) : ?>
          ...
          <?php foreach ($data as $rs) : ?>
            <tr id="row-<?php echo $rs->id; ?>">
              ...
              <td></td>
              <td class="middle text-center"><?php echo $rs->date_upd; ?></td>
              <td class="middle text-center"><?php echo empty($rs->update_user) ? $rs->user : $rs->update_user; ?></td>
            </tr>
            <?php $no++; ?>
          <?php endforeach; ?>
        <?php endif; ?>
...
<script src="<?php echo base_url(); ?>scripts/masters/product_gender.js?v=<?php echo date('Ymd'); ?>"></script>
