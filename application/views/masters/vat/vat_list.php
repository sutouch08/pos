	...
	<div class="row">
		...
		<div class="col-lg-1 col-md-1-harf col-sm-1-harf col-xs-6 padding-5">
			...
			<select class="form-control input-sm filter" name="type">
				...
				<option value="S" <?php echo is_selected('S', $type); ?>>Sales</option>
				<option value="P" <?php echo is_selected('P', $type); ?>>Purchase</option>
			</select>
		</div>
...
<?php if ($this->pm->can_add) : ?>
	<?php $this->load->view('masters/vat/vat_add_control'); ?>
<?php endif; ?>
		...
		<table class="table table-striped tableFixHead dataTable border-1">
			<thead>
				<tr>
					...
					<th class="fix-width-80 middle text-right">อัตรา (%)</th>
...
<script id="edit-row-template" type="text/x-handlebarsTemplate">
	<tr id="edit-row-{{id}}">
      ...
      <button type="button" class="btn btn-minier btn-default" onclick="cancel({{id}})">
		...
		<td class="middle">
			<select class="form-control input-sm e" id="type-{{id}}">
				{{{typeOptions}}}
			</select>
		</td>
		<td class="middle text-center">
			<input type="number" class="form-control input-sm e text-right" id="rate-{{id}}" value="{{rate}}" min="0" max="100" step="0.01" />
		</td>
