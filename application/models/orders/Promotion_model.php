...
class Promotion_model extends CI_Model
{
  ...
  public function get_item_discount($item_code, $customer_code, $qty, $payment_code, $channels_code, $date = '', $temp_id)
	{
    ...
    $this->load->model('orders/order_pos_model');
      ...
      'id_policy' => NULL
		...
		if( ! empty($pd->code) && ! empty($cs->code))
		{
			$qr  = "SELECT DISTINCT
							r.id, r.id_policy, r.item_price,
      ...
      //
        ...
        $dis_policy = NULL;
          ...
          if($isSetMin)
          {
            ...
            {
              //---- คำนวณยอดสั่งใหม่
              $items_in_rule = $this->get_items_in_rule($rs->id);

              if( ! empty($items_in_rule))
              {
                $Qty = $this->get_sum_qty_by_rule_items($temp_id, $item_code, $items_in_rule);
                ...
                //---- ถ้าจำนวนถึง
                if($rs->qty > 0 && $rs->qty > $Qty)
                {
                  $skip = TRUE;
                }

                if($rs->amount > 0 && $rs->amount > $amount)
                {
                  $skip = TRUE;
                }
              }
  ...
  public function get_items_in_rule($rule_id)
  {
    $rs = $this->db
    ->select('style_code')
    ->where('id_rule', $rule_id)
    ->get('discount_rule_product_style');
  ...
  public function get_sum_qty_by_rule_items($temp_id, $item_code, array $ds = array())
  {
    if( ! empty($ds))
    {
      $rs = $this->db
      ->select_sum('qty')
      ->where('order_temp_id', $temp_id)
      ->where('product_code !=', $item_code)
      ->where_in('style_code', $ds)
      ->get('order_pos_temp_detail');
