...
function resizeDisplay() {
	...
	let pageContentHeight = height - (53); //(navHeight + searchHeight + 50);

	let billTableHeight = pageContentHeight - (searchHeight + 65); //(195); //-- 155
	let billViewHeight = pageContentHeight - (searchHeight) //(75);
...
function bill_list() {
	...
	window.location.href = HOME + 'bill_list/'+shop_id;
}
...
function viewSo(code) {
	...
	let center = (window.innerWidth - width)/2;
	let prop = "width="+width+", height="+height+", left="+center+", scrollbars=yes";
	let target = BASE_URL + 'orders/sales_order/view_detail/'+code;
	window.open(target, '_blank', prop);
}
...
function viewInvoice(code) {
	...
	let center = (window.innerWidth - width)/2;
	let prop = "width="+width+", height="+height+", left="+center+", scrollbars=yes";
	let target = BASE_URL + 'orders/order_invoice/view_detail/'+code;
	window.open(target, '_blank', prop);
}
...
function createDeliveryByChannelsAndSales() {
	let shop_id = $('#shop_id').val();
	let bills = [];
	...
	function() {
		setTimeout(() => {
			...
			$.ajax({
				url:BASE_URL + 'orders/order_pos/create_delivery_by_channels_and_sale',
				type:'POST',
				cache:false,
				data: {
					'data' : JSON.stringify(data)
				...
				success:function(rs) {
					...
					if(isJson(rs)) {
						...
						if(ds.status == 'success') {
							swal({
								title:'Success',
								text:'สร้างเอกสารตัดยอดเรียบร้อยแล้ว',
								type:'success'
							}, function() {
								window.location.reload();
							});
